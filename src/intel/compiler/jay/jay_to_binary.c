               ...
               bool force_hi)
{
   ...
   if (jay_is_imm(d)) {
      ...
      unsigned phys_reg = (reg >> 1) / 16;
      offset_B = ((reg >> 1) % 16) * 4;

      if (d.file == UGPR) {
         R = gen_grf(phys_reg, 0);
      } else {
         R = gen_accumulator(phys_reg);
      }
      ...
      if (d.file == GPR) {
         R = gen_restride(gen_grf(phys_reg, 0), 8, 8, 1);
      } else {
         R = gen_restride(gen_accumulator(phys_reg), 8, 8, 1);
      }
