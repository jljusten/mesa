                    ...
                    int base_offset)
{
   ...
   return gen_lsc_ex_desc_encode(devinfo, &ex_desc, NULL);
}
...
{
   if (name == "eq") return GEN_CONDITION_EQ;
   if (name == "ne") return GEN_CONDITION_NE;
   if (name == "gt") return GEN_CONDITION_GT;
   if (name == "ge") return GEN_CONDITION_GE;
   if (name == "lt") return GEN_CONDITION_LT;
   if (name == "le") return GEN_CONDITION_LE;
   if (name == "ov") return GEN_CONDITION_OV;
   if (name == "un") return GEN_CONDITION_UN;
...
math_function_from_name(std::string_view name)
{
   ...
   if (name == "sqt")       return GEN_MATH_SQRT;
...
sfid_from_name(const intel_device_info *devinfo, std::string_view name)
{
   ...
   if (name == "smpl")    return GEN_SFID_SAMPLER;
   if (name == "gtwy")
...
private:
   ...
   struct operand_state {
      ...
      bool has_notification_subreg = false;
      unsigned notification_subreg_index = 0;
   };
   ...
   static bool can_omit_structured_cf_branch_targets(gen_opcode op)
   {
      switch (op) {
      case GEN_OP_IF:
      case GEN_OP_ELSE:
   ...
   bool parse_instruction(std::string_view line, unsigned line_no)
   {
      ...
      case GEN_FORMAT_BRANCH_ONE_SRC:
         c.skip_ws();
         if (can_omit_structured_cf_branch_targets(inst.opcode) &&
             (c.eof() || c.peek() == '{'))
            break;
         if (!require_operand_separator(c, line_no))
            return false;
   ...
   bool looks_like_lsc_symbolic_opcode(const line_cursor &c)
   {
      ...
      if (!lsc_opcode_is_symbolic_name(parts[0]))
         return false;
                                  ...
                                  unsigned line_no)
   {
      ...
      if (sfid != GEN_SFID_SLM && sfid != GEN_SFID_TGM && sfid != GEN_SFID_UGM &&
         ...
         if (sfid == GEN_SFID_URB) {
            error(line_no, "LSC symbolic URB syntax requires Xe2+");
         } else {
      ...
      if (info.sfid == GEN_SFID_URB && info.addr_type != LSC_ADDR_SURFTYPE_FLAT) {
         error(line_no, "URB symbolic syntax does not take a surface selector");
         return false;
      }
                                ...
                                unsigned line_no)
   {
      ...
      if (!parse_send_reg(c, inst.dst, &dst_length, line_no))
         return false;
      if (!require_operand_separator(c, line_no))
         return false;
      ...
      const unsigned ex_mlen = lsc_send_has_symbolic_src1(info.op) ?
         lsc_symbolic_src1_length(devinfo, info, exec_size) : 0;
      const unsigned rlen =
         lsc_symbolic_response_length(devinfo, info, exec_size, !is_null(inst.dst));

      /* The mnemonic fully determines mlen/rlen/ex_mlen. The ':N' suffixes
       * on the operands are optional; when present they must match the
       * mnemonic-implied values. */
      if (dst_length >= 0 && (unsigned)dst_length != rlen) {
         errorf(line_no,
                "dst length ':%d' does not match mnemonic-implied rlen=%u",
                dst_length, rlen);
         return false;
      }
      if (src0_length >= 0 && (unsigned)src0_length != mlen) {
         errorf(line_no,
                "src0 length ':%d' does not match mnemonic-implied mlen=%u",
                src0_length, mlen);
         return false;
      }
      if (src1_length >= 0 && (unsigned)src1_length != ex_mlen) {
         errorf(line_no,
                "src1 length ':%d' does not match mnemonic-implied ex_mlen=%u",
                src1_length, ex_mlen);
         return false;
      }
                  ...
                  unsigned line_no)
   {
      ...
      auto dst_type = parse_type_if_present(c, line_no);
      if (!dst_type)
         return false;
                  ...
                  unsigned line_no)
   {
      ...
      if (looks_like_number(c)) {
         ...
         auto src_type = parse_type_if_present(c, line_no);
         if (!src_type)
            return false;
         src.type = *src_type;
   ...
   {
      if (c.consume("r[")) {
         if (send_grf_subreg && c.consume('s')) {
            unsigned idx = 0;
            if (!c.take_uint(idx)) {
      ...
      return parse_arf_base(c, op, st, line_no);
   }

   bool parse_arf_base(line_cursor &c, gen_operand &op, operand_state &st,
                       unsigned line_no)
   {
      op.file = GEN_ARF;
      ...
      if (c.consume('n')) {
         ...
         if (c.consume('.')) {
            st.has_notification_subreg = true;
            if (!c.take_uint(st.notification_subreg_index)) {
               error(line_no, "expected notification count subregister");
               return false;
            }
         }
         return true;
   ...
   /* Returns the parsed type, or GEN_TYPE_UD if no ':type' suffix is
    * present.  Returns an empty optional on parse error.
    */
   std::optional<gen_reg_type>
   parse_type_if_present(line_cursor &c, unsigned line_no)
   {
      if (!c.consume(':'))
         return GEN_TYPE_UD;
      ...
      gen_reg_type type = type_from_name(name);
                            ...
                            bool send_grf_subreg) const
   {
      ...
      if (op.file == GEN_ARF && !op.indirect &&
          ...
          st.has_notification_subreg) {
         const unsigned type_size = MAX2(gen_type_size_bytes(op.type), 1u);
         op.subnr = st.notification_subreg_index * type_size;
      }
   ...
   bool parse_immediate(line_cursor &c, gen_operand &op, unsigned line_no)
   {
      ...
      op.file = GEN_IMM;
      auto op_type = parse_type_if_present(c, line_no);
      if (!op_type)
         return false;
      op.type = *op_type;
                       ...
                       unsigned line_no)
   {
      ...
      if (op.file == GEN_ARF && !op.indirect &&
          (op.nr & 0xf0) == GEN_ARF_SCALAR && c.line[start] == 's') {
         error(line_no, "scalar send payload register must use r[sN.subnr]");
         return false;
      }
                              ...
                              unsigned line_no)
   {
      ...
      if (note.size() >= 2 && note[0] == '@') {
         /* Bare "@<regdist>" form, used on pre-Xe-HPG platforms where the
          * hardware only has a single in-order pipe, so no pipe letter is
          * emitted.
          */
         inst.swsb.pipe = GEN_PIPE_NONE;

         unsigned long long regdist = 0;
         if (!parse_unsigned_token(note.substr(1), regdist)) {
   ...
   void emit_insts()
   {
      ...
      params->insts = rzalloc_array(params->mem_ctx, gen_inst, insts.size());

      for (unsigned i = 0; i < insts.size(); i++)
         params->insts[i] = insts[i];
   }
