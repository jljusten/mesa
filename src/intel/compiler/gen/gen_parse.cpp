...
private:
      ...
      case GEN_FORMAT_BRANCH:
         if (inst.opcode == GEN_OP_BRC) {
            ...
            if (!c.eof() && c.peek() != '{') {
               if (!require_operand_separator(c, line_no))
                  return false;
               if (!parse_src(c, inst.src[1], st.src_has_swizzle[1], false, 1,
                              inst, line_no))
                  return false;
            }
         } else if (const unsigned num_sources = gen_inst_num_sources(devinfo, &inst)) {
            for (unsigned i = 0; i < num_sources; i++) {
               if (!require_operand_separator(c, line_no))
                  return false;
               if (!parse_src(c, inst.src[i], st.src_has_swizzle[i], false, i,
         ...
         } else if (!parse_branch_targets(c, inst, line_no)) {
            return false;
         }
         break;
   ...
   static bool looks_like_lsc_symbolic_opcode(const line_cursor &c)
   {
      ...
      size_t end = start;
      while (end < c.line.size() && !isspace((unsigned char)c.line[end]) &&
             c.line[end] != '{')
         end++;
   ...
   bool parse_lsc_symbolic_opcode(line_cursor &c, gen_inst &inst,
                                  ...
                                  unsigned line_no)
   {
      const std::string_view word = c.take_until_space_or('{');
      std::vector<std::string_view> parts;
      ...
      const int sfid = sfid_from_name(parts[1]);
   ...
   bool parse_opcode(line_cursor &c, gen_inst &inst, unsigned line_no)
   {
      ...
      while (start < word.size()) {
         ...
         if (suffix == "sat") {
            ...
            const int sfid = sfid_from_name(suffix);
            if (sfid < 0) {
   ...
   bool parse_branch_targets(line_cursor &c, gen_inst &inst, unsigned line_no)
   {
      while (true) {
         c.skip_ws();
         if (c.eof() || c.peek() == '{')
         ...
         if (c.consume("jip:")) {
            is_uip = false;
         } else if (c.consume("uip:")) {
            is_uip = true;
         } else {
            error(line_no, "expected branch target");
            return false;
         }
         ...
         if (is_ident_start(tok[0])) {
            label_uses.push_back({ (int)insts.size(), is_uip, line_no, tok });
         } else {
            unsigned long long v = 0;
            if (!parse_unsigned_token(tok, v)) {
            ...
            if (is_uip)
               inst.branch.uip = (int32_t)(uint32_t)v;
            else
               inst.branch.jip = (int32_t)(uint32_t)v;
         }
      }
   }
