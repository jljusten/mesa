...
{
   /* Any JIPs and UIPs provided by the input are absolute indices
    * in the array.  At the end they will be converted to relative
    * byte offsets.
    */
   ...
   for (int idx = 0; idx < num_insts - 1; idx++) {
      ...
      case GEN_OP_WHILE: {
          ...
          */
         const int jip_idx = inst->branch.jip;
         assert(jip_idx < idx);
         infos.push_back({
            .inst = inst,
            .idx = idx,
   ...
   /* Finally, set actual offsets into the instructions
    * unless there's already a value set by the caller.
    */
   for (auto &info : infos) {
      ...
      case GEN_OP_CONTINUE:
      case GEN_OP_HALT:
         if (inst->branch.uip == 0)
            inst->branch.uip = info.uip_idx;
         inst->branch.uip = 16 * (inst->branch.uip - idx);
         assert(inst->branch.uip > 0);
         FALLTHROUGH;

      case GEN_OP_WHILE:
      case GEN_OP_ENDIF:
         if (inst->branch.jip == 0)
            inst->branch.jip = info.jip_idx;
         inst->branch.jip = 16 * (inst->branch.jip - idx);
         assert(inst->opcode == GEN_OP_WHILE || inst->branch.jip > 0);
         break;
