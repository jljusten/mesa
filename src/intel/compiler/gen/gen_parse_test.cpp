...
enum gen_print_send_syntax {
   GEN_PRINT_SEND_SYNTAX_TRANSLATED = 0,
   GEN_PRINT_SEND_SYNTAX_RAW,
};
...
struct gen_print_options {
   ...
   gen_print_send_syntax send_syntax = GEN_PRINT_SEND_SYNTAX_TRANSLATED;
};
                 ...
                 gen_print_options opts = {})
{
   ...
   if (opts.send_syntax == GEN_PRINT_SEND_SYNTAX_RAW) {
      params.flags = (gen_print_flags)(params.flags | GEN_PRINT_RAW_SENDS);
   }
   params.insts = insts;
...
first_error(const parsed_program &p)
{
   ...
   return std::to_string(p.errors[0].offset) + ": " + p.errors[0].msg;
}
...
TEST(GenParse, RoundTripsCanonicalInstructionStrings)
{
   ...
   const test_case cases[] = {
      ...
      },
      {
         "raw_send_with_swsb",
         "tgl",
         "        send.ugm (1|M0)          r40      r35:1  null:0  a0.2        0x2229E500        {I@2,$5}\n",
         { .flags = (gen_print_flags)(GEN_PRINT_VERBOSE | GEN_PRINT_RAW_SENDS) },
      },
      {
         "raw_send_exbso",
         "mtl",
      ...
      },
      {
         "canonical_send_eot",
         "skl",
         "(W)     send.ts/btd (8|M0)       null     r127:1 null:0  0x00000000  0x02000010        {EOT}\n",
         { .flags = GEN_PRINT_VERBOSE },
      },
      {
         "lsc_ugm_symbolic",
...
TEST(GenParse, ParsesBranchControlSuffix)
{
   ...
   EXPECT_EQ(parsed.insts[0]->branch.jip, 16);
   EXPECT_EQ(parsed.insts[0]->branch.uip, 16);
}
...
TEST(GenParse, NormalizesLegacySendEotDescriptor)
{
   ...
   ASSERT_TRUE(parse_program(
      &devinfo,
      "(W) send.ts/btd (8) null r127:1 null:0 0x00000000 0x82000010 {EOT}\n",
      parsed)) << first_error(parsed);
