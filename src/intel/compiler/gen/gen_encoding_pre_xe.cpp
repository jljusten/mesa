   ...
   encode(const gen_inst *inst, gen_raw_inst *raw)
   {
      ...
      case GEN_FORMAT_BRANCH: {
         ...
         case GEN_OP_JMPI: {
            ...
            if (inst->src[0].file == GEN_IMM) {
               if (gen_type_size_bytes(inst->src[0].type) < 8)
                  set(bits(IMM_32), inst->src[0].imm & 0xFFFFFFFF);
               else
         ...
         case GEN_OP_BRC:
            if (inst->src[0].file == GEN_IMM || inst->src[1].file == GEN_IMM) {
               ...
               set(BRANCH_UIP, inst->src[0].imm & 0xFFFFFFFF);
               set(BRANCH_JIP, inst->src[1].imm & 0xFFFFFFFF);
            } else {
               set(bits(SRC0_FILE), encode_file(inst->src[0].file));
               set(bits(SRC0_TYPE), encode_type(inst->src[0].file, inst->src[0].type));
...
private:
   ...
   encode_operand_controls()
   {
      ...
      if (desc->format == GEN_FORMAT_BRANCH) {
         ...
         set(bits(SRC0_TYPE), encode_type(GEN_IMM, GEN_TYPE_D));
      } else {
         set(bits(SRC0_FILE), encode_file(inst->src[0].file));
         set(bits(SRC0_TYPE), encode_type(inst->src[0].file, inst->src[0].type));
      }
   ...
   decode(gen_inst *inst, const gen_raw_inst *raw)
   {
      ...
      case GEN_FORMAT_BRANCH: {
         ...
         case GEN_OP_JMPI:
            ...
            if (inst->src[0].file == GEN_IMM) {
               if (gen_type_size_bytes(inst->src[0].type) < 8)
                  inst->src[0].imm = get(bits(IMM_32));
               else
         ...
         case GEN_OP_BRC:
            if (inst->src[0].file == GEN_IMM) {
               /* Immediate BRC uses raw Src0 for UIP and raw Src1 for JIP. */
               inst->src[0].imm = get(BRANCH_UIP);

               inst->src[1].file = GEN_IMM;
               inst->src[1].type = inst->src[0].type;
               inst->src[1].imm = get(BRANCH_JIP);
            } else {
               decode_operand_src(bits(SRC0_OPERAND), 0);
