...
typedef struct gen_scan_raw_layout_params {
   ...
   /* Optional per-instruction output arrays.  When non-NULL, each array must
    * have at least num_insts entries.  If the right num_insts is unknown,
    * using arrays with size `raw_bytes_size/8` is always sufficient.
    *
...
typedef struct gen_parse_params {
   ...
   /* For parse errors, `offset` is the 1-based input line number. */
   gen_error *errors;
   int        num_errors;
} gen_parse_params;
