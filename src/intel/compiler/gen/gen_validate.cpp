...
private:
   ...
   static bool
   is_valid_exec_size(unsigned exec_size)
   {
      switch (exec_size) {
      case 1:
      case 2:
   ...
   invalid_values()
   {
      ...
      ERROR_IF(!is_valid_exec_size(inst->exec_size),
   ...
   branch_restrictions()
   {
      ...
      case GEN_OP_BRC:
         if (is_register_file(inst->src[0].file)) {
   ...
   instruction_restrictions()
   {
      ...
      if (inst->opcode == GEN_OP_DPAS) {
         ...
         auto valid_subbyte = [](unsigned subbyte) {
            return subbyte <= 2;
         };

         auto bits_per_element = [&](gen_reg_type type, unsigned subbyte) {
            if (!valid_subbyte(subbyte))
               return 1u;
            return MAX2(1u, gen_type_size_bits(type) >> subbyte);
         };
         ...
         if (src1_type != GEN_TYPE_B && src1_type != GEN_TYPE_UB) {
            ...
            ERROR_IF(!valid_subbyte(src1_subbyte),
         ...
         if (src2_type != GEN_TYPE_B && src2_type != GEN_TYPE_UB) {
            ...
            ERROR_IF(!valid_subbyte(src2_subbyte),
                     "DPAS src2 sub-byte precision is invalid.");
         }

         const unsigned src1_bits_per_element = bits_per_element(src1_type, src1_subbyte);
         const unsigned src2_bits_per_element = bits_per_element(src2_type, src2_subbyte);
      ...
      if (inst->opcode == GEN_OP_OR ||
         ...
         ERROR_IF(inst->cmod == GEN_CONDITION_O || inst->cmod == GEN_CONDITION_U,
                  "Logic instructions must not use O or U conditional modifiers.");
      }
   ...
   send_descriptor_restrictions()
   {
      ...
      case GEN_SFID_URB:
         if (devinfo->ver < 20) {
            enum {
               GFX7_URB_OPCODE_ATOMIC_MOV  = 4,
               GFX7_URB_OPCODE_ATOMIC_INC  = 5,
               GFX8_URB_OPCODE_ATOMIC_ADD  = 6,
               GFX8_URB_OPCODE_SIMD8_WRITE = 7,
               GFX8_URB_OPCODE_SIMD8_READ  = 8,
               GFX125_URB_OPCODE_FENCE     = 9,
            };
   ...
   register_region_special_restrictions()
   {
      ...
      auto check_source = [&](unsigned src_idx) {
