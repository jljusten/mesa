...
print_disasm_help(FILE *fp)
{
   fprintf(fp,
           ...
           "      --translated-sends  Print translated ops instead of raw SEND instructions\n"
           "\n"
           "If INPUT is provided positionally, it is used as the input path.\n"
           "\n"
           "Example:\n"
...
print_syntax_help(FILE *fp)
{
   fprintf(fp,
           ...
           "      '<v;w,h>' is a source region; the destination uses a stride like '<1>'\n"
           "\n"
...
read_input(const std::string &path, std::string *out)
{
   ...
   *out = std::string(std::istreambuf_iterator<char>(stream), {});
   if (stream.bad())
      failf("failed to read input '%s': %s",
            path == "-" ? "<stdin>" : path.c_str(), strerror(errno));
}

static void
write_binary_output(FILE *fp, const void *data, size_t size)
{
...
static void
write_output_bytes(const std::string &path, const std::string &data)
{
   FILE *output = open_output(path, true);
   write_binary_output(output, data.data(), data.size());
...
cmd_asm(int argc, char **argv)
{
   ...
   if (parse_params.num_insts == 0) {
      write_output_bytes(output_path, {});
      ralloc_free(mem_ctx);
      return 0;
   }

   const int raw_bytes_size = parse_params.num_insts > 0 ?
      parse_params.num_insts * (int)sizeof(gen_raw_inst) : 1;

   gen_encode_params encode_params = {};
   encode_params.devinfo = &devinfo;
   encode_params.insts = (const gen_inst **)parse_params.insts;
   encode_params.num_insts = parse_params.num_insts;
   encode_params.mem_ctx = mem_ctx;
   encode_params.raw_bytes = ralloc_size(mem_ctx, raw_bytes_size);
   ...
   write_output_bytes(output_path,
                      std::string((const char *)encode_params.raw_bytes,
...
cmd_disasm(int argc, char **argv)
{
   ...
   static const struct option long_options[] = {
      ...
      {"translated-sends", no_argument, NULL, 1000},
      {},
   };
   ...
   bool *was_compacted = decode_params.num_insts > 0 ?
      ralloc_array(mem_ctx, bool, decode_params.num_insts) : NULL;

   gen_scan_raw_layout_params layout_params = {
      .raw_bytes = decode_params.raw_bytes,
      .raw_bytes_size = decode_params.raw_bytes_size,
      .was_compacted = was_compacted,
      .num_insts = decode_params.num_insts,
   };
   const bool ok = gen_scan_raw_layout(&layout_params);
   assert(ok);
   if (!ok) {
      ralloc_free(mem_ctx);
      failf("raw layout failed");
   }
   assert(layout_params.num_insts == decode_params.num_insts);

   bool any_compacted = false;
   for (int i = 0; i < layout_params.num_insts; i++)
      any_compacted = any_compacted || was_compacted[i];

   if (any_compacted) {
      fprintf(stderr,
              "gentool disasm: warning: compacted instructions are currently shown as\n"
              "placeholder NOPs with a {Compacted} annotation\n");
   }

   gen_error *validation_errors = NULL;
   int num_validation_errors = 0;
   bool valid = true;
   if (decode_params.num_insts > 0) {
      gen_validate_params validate_params = {};
      validate_params.devinfo = &devinfo;
      validate_params.insts = (const gen_inst **)decode_params.insts;
      validate_params.num_insts = decode_params.num_insts;
      validate_params.mem_ctx = mem_ctx;

      valid = gen_validate(&validate_params);
      validation_errors = validate_params.errors;
      num_validation_errors = validate_params.num_errors;
   }
   ...
   print_params.errors = validation_errors;
   print_params.num_errors = num_validation_errors;
   print_params.was_compacted = layout_params.was_compacted;
   gen_print(&print_params);
