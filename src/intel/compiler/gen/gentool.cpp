...
print_disasm_help(FILE *fp)
{
   fprintf(fp,
           ...
           "      --raw-sends         Print raw SEND instructions instead of translated ops\n"
           "\n"
           "If INPUT is provided positionally, it is used as the input path.\n"
           "\n"
           "Example:\n"
...
cmd_disasm(int argc, char **argv)
{
   ...
   static const struct option long_options[] = {
      ...
      {"raw-sends", no_argument,       NULL, 1000},
      {},
   };
   ...
   while (true) {
      ...
      case 'v':
         ...
         break;
      case 1000:
         print_flags = (gen_print_flags)(print_flags | GEN_PRINT_RAW_SENDS);
         break;
      case '?':
      default:
         failf("unknown option '%s'", argv[optind - 1]);
   ...
   for (int i = 0; i < decode_params.num_insts; i++)
      any_compacted = any_compacted || decode_params.was_compacted[i];
   ...
   print_params.was_compacted = decode_params.was_compacted;
   gen_print(&print_params);
