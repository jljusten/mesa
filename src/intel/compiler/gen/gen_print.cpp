...
struct gen_printer {
   ...
   print(const gen_inst *inst, int idx)
   {
      ...
      print_operands(inst, idx);
      print_annotation(inst, idx);
      print_send_comment(inst);
      newline();
   }
...
private:
   ...
   has_cond_modifier(const gen_inst *inst) const
   {
      ...
      return gen_inst_has_cond_modifier(format, inst) &&
             !(devinfo->ver >= 12 &&
   ...
   void
   print_operands(const gen_inst *inst, int idx)
   {
      const gen_format format = gen_inst_format(inst->opcode);
      ...
      case GEN_FORMAT_BRANCH_ONE_SRC:
         ...
         if (inst->src[0].file == GEN_IMM)
            print_branch_target(idx, inst->src[0].imm);
         else
            print_src(inst->src[0], inst->align16);
         break;
      ...
      case GEN_FORMAT_BRANCH_TWO_SRC:
         ...
         field_sep(66);
         text("uip:");
         print_branch_target(idx, inst->src[1].imm);
         break;
...
#undef BRW_SWIZZLE_XYZW
   ...
   bool
   print_annotation(const gen_inst *inst, int idx)
   {
   ...
   void
   print_send_comment(const gen_inst *inst)
   {
      lsc_symbolic_info lsc;
...
static const char *
normalize_annotation_text(const char *text)
{
...
{
   a = normalize_annotation_text(a);
   b = normalize_annotation_text(b);
...
{
   text = normalize_annotation_text(text);
   if (!text)
      return;
                      ...
                      const bool *was_compacted)
{
   ...
   for (int i = 0; i < num_insts; i++) {
      ...
      if (!annotation_text_equal(last_annotation, annotation))
         print_annotation_text(fp, annotation);
      last_annotation = normalize_annotation_text(annotation);
...
gen_print(gen_print_params *params)
{
   ...
   for (int i = 0; i < num_insts; i++) {
      ...
      if (inst->src[0].file == GEN_IMM && inst->src[0].imm) {
         const int target = branch_target_index(i, inst->src[0].imm);
         if (target >= 0 && target <= num_insts)
            targets.push_back(target);
      }

      if (format == GEN_FORMAT_BRANCH_TWO_SRC &&
          inst->src[1].file == GEN_IMM && inst->src[1].imm) {
         const int target = branch_target_index(i, inst->src[1].imm);
         if (target >= 0 && target <= num_insts)
            targets.push_back(target);
      }
   }
