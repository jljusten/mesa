...
/* TODO: Take advantage of gen_format, move to encoder/decoder. */
uint32_t
...
struct gen_encoder {
   ...
   encode(const gen_inst *inst, gen_raw_inst *raw)
   {
      ...
      } else if (gen_inst_has_cond_modifier(desc->format, inst) &&
...
gen_scan_raw_layout(gen_scan_raw_layout_params *params)
{
   ...
   while (raw < raw_end) {
      ...
      if (params->offsets)
         params->offsets[count] = (raw - raw_start) * sizeof(*raw);
      if (params->was_compacted)
         params->was_compacted[count] = compact;
