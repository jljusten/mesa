                     ...
                     const gen_inst *inst)
{
   ...
   case GEN_FORMAT_BRANCH: {
      if (inst->opcode == GEN_OP_JMPI || inst->opcode == GEN_OP_BRD) {
         return 1;
      } else if (inst->opcode == GEN_OP_BRC) {
         return inst->src[0].file == GEN_IMM ||
                inst->src[1].file != GEN_BAD_FILE ? 2 : 1;
      } else {
         /* UIP and JIP comes from branch sub-struct. */
         return 0;
      }
   }
...
gen_swsb_sbid(enum gen_sbid_mode mode, unsigned sbid)
{
                       ...
                       uint32_t raw_ex_desc)
{
   ...
   case LSC_ADDR_SURFTYPE_FLAT:
      ex_desc.flat.base_offset = GET_BITS(raw_ex_desc, 31, 12);
      break;
   ...
   case LSC_ADDR_SURFTYPE_BTI:
      ...
      ex_desc.bti.base_offset = GET_BITS(raw_ex_desc, 23, 12);
      break;
                       ...
                       const gen_lsc_ex_desc *ex_desc)
{
   ...
   case LSC_ADDR_SURFTYPE_FLAT:
      return SET_BITS(ex_desc->flat.base_offset, 31, 12);

   case LSC_ADDR_SURFTYPE_BSS:
   case LSC_ADDR_SURFTYPE_SS:
      return SET_BITS(ex_desc->surface_state.surface_state_index, 31, 6);

   case LSC_ADDR_SURFTYPE_BTI:
      return SET_BITS(ex_desc->bti.index, 31, 24) |
             SET_BITS(ex_desc->bti.base_offset, 23, 12);
...
struct gen_encoder {
   ...
   encode(const gen_inst *inst, gen_raw_inst *raw)
   {
      ...
      if (gen_inst_has_cond_modifier(desc->format, inst)) {
         if (inst->opcode == GEN_OP_BFN) {
            ...
            const unsigned encoded_bfn_cmod =
               inst->cmod == GEN_CONDITION_Z ? 1 :
               inst->cmod == GEN_CONDITION_G ? 2 :
               inst->cmod == GEN_CONDITION_L ? 3 : 0;
      ...
      case GEN_FORMAT_BRANCH: {
         ...
         case GEN_OP_JMPI:
         case GEN_OP_BRD:
            encode_branch_src0(inst->src[0]);
            break;

         case GEN_OP_BRC:
            if (inst->src[0].file == GEN_IMM || inst->src[1].file == GEN_IMM) {
               /* Immediate BRC uses raw Src0 for UIP and raw Src1 for JIP. */
               assert(inst->src[0].file == GEN_IMM);
               assert(inst->src[1].file == GEN_IMM);

               set(E::SRC0_TYPE, encode_type(inst->src[0].file, inst->src[0].type));
               set(E::SRC0_IS_IMM, 1);
               set(E::SRC1_IS_IMM, 1);
               set(E::IMM_HI_32, inst->src[0].imm & 0xFFFFFFFF);
               set(E::IMM_LO_32, inst->src[1].imm & 0xFFFFFFFF);
            } else {
               encode_branch_src0(inst->src[0]);
            }
            break;
   ...
   void
   encode_branch_src0(const gen_operand &src)
   {
      set(E::SRC0_ADDRESS_MODE, src.indirect);
      set(E::SRC0_NEGATE,       src.negate);
      set(E::SRC0_ABS,          src.abs);
      set(E::SRC0_TYPE,         encode_type(src.file, src.type));

      if (src.file == GEN_IMM) {
         set(E::SRC0_IS_IMM, 1);
         set(E::IMM_LO_32, src.imm & 0xFFFFFFFF);
         if (gen_type_size_bytes(src.type) > 4)
            set(E::IMM_HI_32, src.imm >> 32);
         return;
      }

      if (src.indirect)
         encode_indirect_operand(E::SRC0_OPERAND, src);
      else
         encode_direct_operand(E::SRC0_OPERAND, src);

      if constexpr (E::TYPE >= GEN_ENCODING_XE2) {
         set(E::SRC0_OPERAND_EXTRA, src.indirect ? src.addr_imm & 1
                                                 : src.subnr & 1);
      }

      set(E::SRC0_VSTRIDE, encode_vstride(src.region.vstride));
      set(E::SRC0_WIDTH,   encode_width(src.region.width));
      set(E::SRC0_HSTRIDE, encode_hstride(src.region.hstride));
   }
...
struct gen_decoder {
   ...
   decode_many(gen_decode_params *params)
   {
      ...
      while (raw < raw_end) {
         if (gen_raw_is_compact((void *)raw)) {
            ...
            params->was_compacted[decoded] = true;
            decoded++;
            raw += 1;
         } else {
            /* TODO: Error handling. */
   ...
   decode(gen_inst *inst, const gen_raw_inst *raw)
   {
      ...
      if (gen_inst_has_cond_modifier(desc->format, inst)) {
         if (inst->opcode == GEN_OP_BFN) {
            ...
            inst->cmod = encoded_bfn_cmod == 1 ? GEN_CONDITION_Z :
                         encoded_bfn_cmod == 2 ? GEN_CONDITION_G :
                         encoded_bfn_cmod == 3 ? GEN_CONDITION_L :
                                                 GEN_CONDITION_NONE;
         } else {
      ...
      case GEN_FORMAT_BASIC_THREE_SRC: {
         ...
         inst->src[1].region.width = gen_implied_width_for_3src_a1(inst->src[1].region.vstride, inst->src[0].region.hstride);
      ...
      case GEN_FORMAT_BRANCH: {
         ...
         case GEN_OP_JMPI:
         case GEN_OP_BRD:
            decode_branch_src0(inst->src[0]);
            break;

         case GEN_OP_BRC:
            if (get(E::SRC0_IS_IMM) && get(E::SRC1_IS_IMM)) {
               /* Immediate BRC uses raw Src0 for UIP and raw Src1 for JIP. */
               inst->src[0].file = GEN_IMM;
               inst->src[0].type = decode_type(inst->src[0].file, get(E::SRC0_TYPE));
               inst->src[0].imm  = get(E::IMM_HI_32);

               inst->src[1].file = GEN_IMM;
               inst->src[1].type = inst->src[0].type;
               inst->src[1].imm  = get(E::IMM_LO_32);
            } else {
               decode_branch_src0(inst->src[0]);
            }
            break;
   ...
   void
   decode_branch_src0(gen_operand &src)
   {
      src.indirect = get(E::SRC0_ADDRESS_MODE);
      src.negate   = get(E::SRC0_NEGATE);
      src.abs      = get(E::SRC0_ABS);

      if (get(E::SRC0_IS_IMM)) {
         src.file = GEN_IMM;
         src.type = decode_type(src.file, get(E::SRC0_TYPE));
      ...
      if (src.indirect) {
         decode_indirect_operand(E::SRC0_OPERAND, src);
         src.file = GEN_GRF;
         if constexpr (E::TYPE >= GEN_ENCODING_XE2)
            src.addr_imm |= get(E::SRC0_OPERAND_EXTRA);
      } else {
         decode_direct_operand(E::SRC0_OPERAND, src);
         if constexpr (E::TYPE >= GEN_ENCODING_XE2)
            src.subnr |= get(E::SRC0_OPERAND_EXTRA);
      }

      src.region.vstride = decode_vstride(get(E::SRC0_VSTRIDE));
      src.region.width   = decode_width(get(E::SRC0_WIDTH));
      src.region.hstride = decode_hstride(get(E::SRC0_HSTRIDE));
      src.type           = decode_type(src.file, get(E::SRC0_TYPE));
   }
...
gen_decode(gen_decode_params *params)
{
   ...
   params->was_compacted = rzalloc_array(params->mem_ctx, bool, params->num_insts);
