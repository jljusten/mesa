...
gen_validate_test::validate(const gen_inst &inst)
{
   ...
   const gen_inst *insts[] = { &inst };

   gen_validate_params params = {};
   params.devinfo = &devinfo;
   params.insts = insts;
   params.num_insts = ARRAY_SIZE(insts);
...
TEST_P(gen_validate_test, align16_3src_repctrl_must_be_zero_for_64bit_sources)
{
   ...
   const bool has_64bit = devinfo.has_64bit_float || devinfo.has_64bit_int;
   if (!has_64bit)
      return;
...
TEST_P(gen_validate_test, bfn_restrictions)
{
   ...
   for (gen_condition cmod : { GEN_CONDITION_NONE, GEN_CONDITION_Z,
                               GEN_CONDITION_G, GEN_CONDITION_L }) {
...
TEST_P(gen_validate_test, send_eot_payload_restrictions)
{
   ...
   inst.src[1].file = GEN_GRF;
   inst.src[1].nr = 112;
   EXPECT_EQ(devinfo.ver >= 30, validate(inst));
