                         ...
                         int offset, const struct brw_label *root_label)
{
   ...
   if (gen_has_uip(opcode)) {
      pad(file, 16);
      string(file, "JIP: ");
      write_label(file, devinfo, root_label, offset, inst->branch.jip);
      format(file, "(0x%x)", inst->branch.jip);

      pad(file, 38);
      string(file, "UIP: ");
      write_label(file, devinfo, root_label, offset, inst->branch.uip);
      format(file, "(0x%x)", inst->branch.uip);
