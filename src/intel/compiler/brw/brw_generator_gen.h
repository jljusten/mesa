...
class brw_generator_gen
{
public:
   brw_generator_gen(const struct brw_compiler *compiler,
...
private:
   ...
   void generate_broadcast(brw_reg dst, brw_reg src, brw_reg idx);
   void generate_math(brw_reg dst, brw_reg src0, brw_reg src1, gen_math func);
   ...
   gen_operand to_gen(brw_reg r, bool align16 = false);
   gen_opcode to_gen(enum opcode op);
   gen_file to_gen(brw_reg_file file);
   gen_swsb to_gen(tgl_swsb swsb);

   gen_inst *make_empty();
   gen_inst *make(gen_opcode op);
   gen_inst *append(gen_inst *inst);

   gen_inst *append(enum opcode opcode);
   gen_inst *append(enum opcode opcode, brw_reg dst,
                    brw_reg src0);
   gen_inst *append(enum opcode opcode, brw_reg dst,
   ...
   inline gen_inst *append_MOV(brw_reg dst, brw_reg src0) { return append(BRW_OPCODE_MOV, dst, src0); }
