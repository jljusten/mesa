...
gen_region_from_reg(brw_reg r)
{
   uint8_t v = r.vstride == 0                                   ? 0 :
               r.vstride == BRW_VERTICAL_STRIDE_ONE_DIMENSIONAL ? r.vstride :
                                                                  (1 << (r.vstride - 1));
   uint8_t w = 1 << r.width;
...
gen_file
brw_generator_gen::to_gen(brw_reg_file file)
{
   switch (file) {
...
gen_operand
brw_generator_gen::to_gen(brw_reg r, bool align16)
{
   gen_operand o = {};
...
brw_generator_gen::brw_generator_gen(const struct brw_compiler *compiler,
...
brw_generator_gen::~brw_generator_gen()
{
}

void
brw_generator_gen::generate_send(brw_send_inst *inst,
                                 struct brw_reg dst,
                                 struct brw_reg desc,
                                 ...
                                 bool ex_bso)
{
   ...
   gen_inst *gen = append(BRW_OPCODE_SEND);
   gen->send.eot = send->eot;
   gen->send.sfid = send->sfid;

   gen->dst.file = to_gen(dst.file);
   gen->dst.nr = phys_nr(devinfo, dst);
   gen->dst.type = BRW_TYPE_UD;

   gen->src[0] = to_gen(payload);
   gen->src[1] = to_gen(payload2);

   if (!payload2.is_null() && devinfo->ver < 12)
      gen->opcode = GEN_OP_SENDS;
   ...
   if (devinfo->ver >= 20 && gen->send.sfid == BRW_SFID_UGM) {
      gen->send.src1_len = inst->ex_mlen / reg_unit(devinfo);
   }
   ...
   //                                    inst->eot, gather);
   //    if (inst->check_tdr)
   //       gen->opcode = devinfo->ver >= 12 ? GEN_OP_SENDC : GEN_OP_SENDSC;
   // }
}

void
brw_generator_gen::generate_mov_indirect(brw_inst *inst,
                                         struct brw_reg dst,
                                         struct brw_reg reg,
                                         struct brw_reg indirect_byte_offset)
{
   ...
   if (indirect_byte_offset.file == IMM) {
      ...
      if (brw_type_size_bytes(reg.type) > 4 && !devinfo->has_64bit_int) {
         ...
         current_state()->swsb = tgl_swsb_null();
      ...
      gen->no_mask = true;
      gen->pred_control = BRW_PREDICATE_NONE;
      if (devinfo->ver >= 12)
         current_state()->swsb = tgl_swsb_null();
      else
      ...
      if (devinfo->ver >= 12)
         current_state()->swsb = tgl_swsb_regdist(1);
      else
...
void
brw_generator_gen::generate_shuffle(brw_inst *inst,
                                    struct brw_reg dst,
                                    struct brw_reg src,
                                    struct brw_reg idx)
{
...
void
brw_generator_gen::generate_quad_swizzle(const brw_inst *inst,
...
void
brw_generator_gen::generate_barrier(brw_inst *, struct brw_reg src)
{
   gen_inst *gen = append(BRW_OPCODE_SEND,
   ...
   desc |= (uint32_t)BRW_MESSAGE_GATEWAY_SFID_BARRIER_MSG;
   gen->align16 = false;
   gen->send.sfid = BRW_SFID_MESSAGE_GATEWAY;
   gen->send.desc_imm = desc;
   gen->no_mask = true;

   if (devinfo->ver >= 12) {
      current_state()->swsb = tgl_swsb_null();
      append_SYNC(TGL_SYNC_BAR);
   } else {
 ...
 */
void
brw_generator_gen::generate_ddx(const brw_inst *inst,
                                struct brw_reg dst, struct brw_reg src)
{
 ...
 */
void
brw_generator_gen::generate_ddy(const brw_inst *inst,
                                struct brw_reg dst, struct brw_reg src)
{
...
void
brw_generator_gen::generate_broadcast(brw_reg dst, brw_reg src, brw_reg idx)
{
   push_state();
   ...
   if ((src.vstride == 0 && src.hstride == 0) ||
      ...
      push_state();
      current_state()->no_mask = true;
      current_state()->pred_control = BRW_PREDICATE_NONE;
...
void
brw_generator_gen::generate_math(brw_reg dst, brw_reg src0, brw_reg src1,
                                 unsigned math_func)
{
   gen_inst *gen = append(BRW_OPCODE_MATH, dst, src0, src1);
   gen->math.func = math_func;
   // TODO: Remove this hack!
   gen->cond_modifier = (brw_conditional_mod) math_func;
 ...
 */
void
brw_generator_gen::generate_scratch_header(brw_inst *inst,
                                           struct brw_reg dst,
                                           struct brw_reg src)
{
#if 0
...
void
brw_generator_gen::generate_float_controls_mode(unsigned mode, unsigned mask)
{
   ...
   if (devinfo->ver >= 12)
      append_SYNC(TGL_SYNC_NOP);
}

void
brw_generator_gen::enable_debug(const char *shader_name)
{
   debug_flag = true;
...
int
brw_generator_gen::generate_code(const brw_shader &s,
                                 struct genisa_stats *stats)
{
   ...
   foreach_block_and_inst (block, brw_inst, inst, s.cfg) {
      ...
      if (inst->eot && is_accum_used &&
         ...
         gen->exec_size = 16;
         gen->no_mask = true;
         gen->swsb = tgl_swsb_src_dep(swsb);
      ...
      if (inst->eot && intel_needs_workaround(devinfo, 14013672992)) {
         if (tgl_swsb_src_dep(swsb).mode) {
            ...
            // TODO: HAC HACK
            gen->cond_modifier = (brw_conditional_mod) TGL_SYNC_NOP;
            append(gen);
	 ...
	 assert(!inst->conditional_mod ||
...

      {
         ...
         state->align16 = false;
         state->pred_control = inst->predicate;
      ...
      case BRW_OPCODE_SYNC:
         ...
         append_SYNC(tgl_sync_function(src[0].ud));
	 ...
	 break;
      ...
      case BRW_OPCODE_IF: {
         ...
         gen->chan_offset = 0;
         gen->pred_control = BRW_PREDICATE_NORMAL;
         gen->no_mask = false;
	 ...
	 break;
      ...
      case BRW_OPCODE_ENDIF: {
         ...
         if (devinfo->ver == 9 && else_idx) {
            ...
            else_inst->branch.jip = endif_idx;
            else_inst->branch.uip = nop_idx;
	 ...
	 break;
      ...
      case BRW_OPCODE_WHILE: {
         ...
         while_inst->branch.jip = header_idx;
	 ...
	 break;
      ...
      case SHADER_OPCODE_POW:
         ...
         generate_math(dst, src[0], src[1], brw_math_function(inst->opcode));
	 break;
	 ...
	 break;
      ...
      case SHADER_OPCODE_MOV_RELOC_IMM: {
          ...
          */
         const unsigned DEFAULT_PATCH_IMM = 0x4a7cc037;
         append_MOV(dst, retype(brw_imm_ud(DEFAULT_PATCH_IMM), dst.type));
      ...
      case FS_OPCODE_LOAD_LIVE_CHANNELS: {
         ...
         current_state()->exec_size = 1;
         current_state()->swsb = tgl_swsb_dst_dep(swsb, 1);
	 ...
	 break;
      ...
      case SHADER_OPCODE_READ_ARCH_REG:
         if (devinfo->ver >= 12) {
             ...
             */
            if (state->swsb.mode != TGL_SBID_NULL)
               append_SYNC(TGL_SYNC_NOP);
            state->swsb = tgl_swsb_regdist(1);
      ...
      if (inst->conditional_mod) {
         ...
         gen->cond_modifier = inst->conditional_mod;
      }
      ...
      if (INTEL_DEBUG(DEBUG_SWSB_STALL) && devinfo->ver >= 12) {
         current_state()->swsb = tgl_swsb_regdist(1);
         // TODO: Sync with SBID defined above if any!
         append_SYNC(TGL_SYNC_NOP);
      }
   }

   // TODO: Various normalizations.  See if can move them elsewhere.
   for (auto *gen : gen_insts) {
      ...
      // TODO: See if we can drop this now.
      /* Workaround for simulator issue. See 7428e6f86ab5022ba07f562e124642245c63a72f
       * ("i965: Set dest type to UW for several send messages") for details.
       */
      if (devinfo->ver < 12 && (gen->opcode == GEN_OP_SEND ||
                                gen->opcode == GEN_OP_SENDC))
         gen->dst.type = BRW_TYPE_UW;
   }
   ...
   gen_encode_params enc_params = {
      ...
      // TODO: create gen_inst_ptr so const doesn't look bad in the api?
      .insts = (const gen_inst **)gen_insts.data(),
   ...
   if (!gen_encode(&enc_params)) {
      gen_print(devinfo, gen_insts.data(), gen_insts.size(),
                { .errors = enc_params.errors, .num_errors = enc_params.num_errors });
      start_offset = -1;
   } else {
      // TODO: Remove.
      //gen_print(devinfo, gen_insts.data(), gen_insts.size(), {});
   ...
   // TODO: Add some better allocation scheme.
   for (auto gen : gen_insts)
      delete gen;
   gen_insts.clear();
...
void
brw_generator_gen::add_const_data(void *data, unsigned size)
{
...
void
brw_generator_gen::add_resume_sbt(unsigned num_resume_shaders, uint64_t *sbt)
{
...
const unsigned *
brw_generator_gen::get_assembly()
{
...
gen_inst *
brw_generator_gen::make_empty()
{
   // TODO: Do better.
   gen_inst *gen = new gen_inst;
   memset(gen, 0, sizeof(gen_inst));
   return gen;
}

gen_inst *
brw_generator_gen::make(gen_opcode op)
{
   gen_inst *gen = make_empty();
...
gen_inst *
brw_generator_gen::append(gen_inst *gen)
{
...
gen_inst *
brw_generator_gen::append(enum opcode opcode)
{
   return append(make(to_gen(opcode)));
}

gen_inst *
brw_generator_gen::append(enum opcode opcode, brw_reg dst,
                          brw_reg src0)
{
...
gen_inst *
brw_generator_gen::append(enum opcode opcode, brw_reg dst,
                          brw_reg src0, brw_reg src1, brw_reg src2)
{
   gen_inst *gen = append(opcode);
   gen->dst    = to_gen(dst,  gen->align16);
   gen->src[0] = to_gen(src0, gen->align16);
...
gen_inst *
brw_generator_gen::append_SYNC(enum tgl_sync_function func)
{
   ...
   // TODO: HACK HACK
   gen->cond_modifier = (brw_conditional_mod) func;

   return append(gen);
}

gen_inst *
brw_generator_gen::append_NOP()
{
...
int
brw_generator_gen::allocate_output(unsigned size, unsigned alignment)
{
...
int
brw_generator_gen::append_output(void *data, unsigned size, unsigned alignment)
{
...
void
brw_generator_gen::append_reloc(const intel_shader_reloc &r)
{
...
print_raw(const struct brw_isa_info *isa, FILE *fp, const void *raw)
{
   ...
   for (int i = msb; i >= 0; i--) {
      if (i && (i+1) % 16 == 0)
         fprintf(fp, "'");
      fprintf(fp, "%lu", brw_eu_inst_bits((brw_eu_inst *)raw, i, i));
   }
   fprintf(fp, "  (lsb)\n");
}
             ...
             const void *inst)
{
   ...
   bool is_compact = gen_as_raw_compact_inst(isa->devinfo, inst);
   if (is_compact) {
      brw_uncompact_instruction(isa, &uncompacted, (brw_eu_compact_inst*)inst);
      inst = &uncompacted;
   }
   brw_disassemble_inst(fp, isa, (brw_eu_inst *)inst, is_compact, 0, NULL);
}
...
diff_insts(const struct brw_isa_info *isa,
           ...
           const void *encoded)
{
   auto a = gen_as_raw_inst(isa->devinfo, original);
   auto ac = gen_as_raw_compact_inst(isa->devinfo, original);
   auto b = gen_as_raw_inst(isa->devinfo, encoded);
   auto bc = gen_as_raw_compact_inst(isa->devinfo, encoded);

   if (a && b && (memcmp(a, b, sizeof(*a)) == 0))
      return false;

   if (ac && bc && (memcmp(ac, bc, sizeof(*ac)) == 0))
      return false;

   bool compact_diff = (a && bc) || (ac && b);
   ...
   if (compact_diff && (memcmp(&au, &bu, sizeof(au)) == 0)) {
      if (INTEL_GEN_DEBUG(VERBOSE)) {
         printf("Intel gen: Compact vs uncompact mismatch, "
                "but uncompact instructions match\n");
      }
      return false;
   }
   ...
   fprintf(fp, "                       "
           ...
           "\n");
   fprintf(fp, "ORIGINAL BITS: ");
   print_raw(isa, fp, original);

   fprintf(fp, "ENCODED BITS:  ");
   print_raw(isa, fp, encoded);

   if (ac) {
      fprintf(fp, "ORIG UNCOMP:   ");
      print_raw(isa, fp, &au);
   }

   if (bc) {
      fprintf(fp, "ENC UNCOMP:    ");
      print_raw(isa, fp, &bu);
   }
   ...
   for (int i = 127; i >= 0; i--) {
      if (i && (i+1) % 16 == 0)
         fprintf(fp, " ");
      if (brw_eu_inst_bits((brw_eu_inst *)&au, i, i) !=
          brw_eu_inst_bits((brw_eu_inst*)&bu, i, i))
         fprintf(fp, "^");
      else
         fprintf(fp, " ");
   }
